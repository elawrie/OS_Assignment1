// for tasks that have the same priority, they get scheduled in round robin
// dont do round robin for tasks that have diff priority 
...
int timeQuantum = 10; // Adjust this value as needed
...
// function to pick the next task with the highest priority and apply round robin
Task* pickNextTask(struct node **head) {
    ...
    // Update the priority for round-robin
    nextTask->priority -= timeQuantum;

    // If priority becomes non-positive, remove the node from the list
